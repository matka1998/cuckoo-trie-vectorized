...
void print_entry(cuckoo_trie* trie, uint64_t bucket, int cell) {
	ct_entry_descriptor entry = (ct_entry_descriptor) {
		.common = (ct_common_header*)&(trie->buckets[bucket].common_cells[cell]),
		.type_specific = (ct_type_specific_entry*)&(trie->buckets[bucket].type_specific_cells[cell])
	};
	// ct_entry* entry = (ct_entry*) &(trie->buckets[bucket].cells[cell]);
	printf("\tBucket %lu cell %d (C:%p T:%p): %s tag=0x%x last_symbol=0x%x\n",
					   bucket, cell, entry.common, entry.type_specific, type_name(entry_type_descriptor(entry)), entry_type_descriptor(entry),
					   entry.common->last_symbol);
}
...
int verify_bitmap_children(cuckoo_trie* trie, uint64_t bucket, int cell) {
	...
	if (entry_is_secondary_descriptor(entry))
		primary_bucket = unmix_bucket(trie, bucket, entry_tag_descriptor(entry));

	prefix_hash = (primary_bucket << TAG_BITS) + entry_tag_descriptor(entry);

	for (child = 0;child < FANOUT + 1;child++) {
		if (!get_bit(entry.type_specific->child_bitmap, child))
			continue;

		uint64_t child_prefix_hash = accumulate_hash(trie, prefix_hash, child);
		ct_entry_local_copy unused;
		ct_entry_descriptor result = find_entry_in_pair_by_parent(trie, &unused,
																hash_to_bucket(child_prefix_hash),
																hash_to_tag(child_prefix_hash),
																child,
																entry_color_descriptor(entry));
		if (!result.common) {
...
int verify_jump_child(cuckoo_trie* trie, uint64_t bucket, int cell) {
	...
	for (i = 0;i < entry_jump_size_descriptor(entry);i++)
		prefix_hash = accumulate_hash(trie, prefix_hash, get_jump_symbol_descriptor(entry, i));

	child = find_entry_in_pair_by_color(trie, &unused, hash_to_bucket(prefix_hash),
										hash_to_tag(prefix_hash), entry_child_color_descriptor(entry));
	if (!child.common) {
		printf("Error: Jump child doesn't exist\n");
		return 0;
	}

	if (entry_type_descriptor(child) == TYPE_LEAF) {
...
int verify_entry(cuckoo_trie* trie, uint64_t bucket, int cell) {
	...
	if (entry_type_descriptor(entry) != TYPE_LEAF) {
		// This entry has a max_leaf field. Verify it.
		char* max_leaf_error = leaf_locator_error(trie, &(entry.type_specific->max_leaf));
		if (max_leaf_error != NULL) {
			printf("Error: max_leaf locator of bucket %lu cell %d is broken (%s).\n",
				   bucket, cell, max_leaf_error);
			print_locator(&(entry.type_specific->max_leaf));
			is_ok = 0;
		}
	}

	if (entry_type_descriptor(entry) == TYPE_BITMAP) {
		if (verify_bitmap_children(trie, bucket, cell) == 0)
			is_ok = 0;
	}
	if (entry_type_descriptor(entry) == TYPE_JUMP) {
		if (verify_jump_child(trie, bucket, cell) == 0)
			is_ok = 0;
	}
	if (entry_type_descriptor(entry) == TYPE_LEAF) {
		int is_max_leaf = (entry.type_specific->next_leaf.primary_bucket == ((uint32_t)-1));
		if (!is_max_leaf) {
			char* next_leaf_error = leaf_locator_error(trie, &(entry.type_specific->next_leaf));
			if (next_leaf_error != NULL) {
				printf("Error: next_leaf locator of bucket %lu cell %d is broken (%s).\n",
					   bucket, cell, next_leaf_error);
				print_locator(&(entry.type_specific->next_leaf));
				is_ok = 0;
			}
		}
	}
...
int verify_linklist(cuckoo_trie* trie) {
	...
	ct_kv* last_kv = NULL;
	ct_entry_locator next = trie_min_leaf(trie).type_specific->next_leaf;
	...
	while (next.primary_bucket != (uint32_t)-1) {
		...
		ct_entry_descriptor leaf = try_follow_locator(trie, &next);
		if (!leaf.common) {
		...
		if (entry_type_descriptor(leaf) != TYPE_LEAF) {
		...
		if (last_kv != NULL) {
			if (kv_key_compare(entry_kv_descriptor(leaf), last_kv) <= 0) {
				printf("Error: Leaf C: %p T: %p key is smaller than previous\n", leaf.common, leaf.type_specific);
				printf("\tKey of leaf:     ");
				print_key(entry_kv_descriptor(leaf));
				printf("\n");
		...
		last_kv = entry_kv_descriptor(leaf);
		linklist_leaves++;
		...
		next = leaf.type_specific->next_leaf;
