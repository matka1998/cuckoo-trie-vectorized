...
// Test that ct_insert returns the appropriate result
void test_insert_split_view(int verbose) {
	const uint64_t num_keys = 5000;
	const uint64_t max_key_len = 16;
	...
	uint8_t* kvs_buf = malloc((kv_required_size(max_key_len, DEFAULT_VALUE_SIZE)) * num_keys);
	uint8_t* buf_pos = kvs_buf;
	cuckoo_trie_split* trie = ct_alloc_split_view(5000);
	...
	for (i = 0;i < num_keys; i++) {
		...
		result = ct_insert_split_view(trie, kv);
		if (result == S_OVERFLOW) {
			printf("Trie full. Inserted %d keys.\n", i - 1);
			printf("OK!\n");
			break;
		}
		if (!ct_verify_trie_split_view(trie)) {
			printf("ERROR! Trie structure is broken.\n");
			break;
		}
...
void test_iter_split_view() {
	...
	uint8_t* buf_pos = kvs_buf;
	ct_kv* next_kv;
	cuckoo_trie_split* trie = ct_alloc_split_view(3000);
	...
	ct_iter_split* iter = ct_iter_alloc_split_view(trie);
	if (!iter) {
		printf("Error: Couldn't allocate iterator\n");
		return;
	}

	ct_iter_goto_split_view(iter, range_start_size, range_start_buf);
	while (expected_rank < num_kvs) {
		next_kv = ct_iter_next_split_view(iter);
		if (next_kv != kv_pointers[expected_rank]) {
			printf("Error: Got incorrect key\n");
			printf("\tStarted from: ");
			print_bytes(range_start_size, range_start_buf);
...
// Writer thread: Insert all keys into the trie
void* mt_insert_lookup_writer_thread_split_view(void* arg) {
	uint64_t i;
	int result;
	uint8_t* buf_pos;
	mt_insert_lookup_ctx_split* ctx = (mt_insert_lookup_ctx_split*)arg;

	buf_pos = ctx->kvs;
	for (i = 0;i < ctx->num_kvs;i++) {
		ct_kv* kv = (ct_kv*)buf_pos;
		result = ct_insert_split_view(ctx->trie, kv);
		if (result != S_OK) {
			printf("Error: ct_insert returned %d in writer\n", result);
			return NULL;
		}
...
// We try to trigger more edge cases by searching for the key that is being inserted
// right now.
void* mt_insert_lookup_reader_thread_split_view(void* arg) {
	uint64_t pos;
	uint64_t i;
	uint64_t step;
	uint8_t* buf_pos;
	...
	while (1) {
		ct_kv* kv = (ct_kv*)buf_pos;
		ct_kv* result = ct_lookup_split_view(ctx->trie, kv_key_size(kv), kv_key_bytes(kv));
...
// Perform lookups in parallel with inserts
void test_mt_insert_lookup_split_view() {
	const uint64_t num_kvs = 10000;
	const uint64_t max_key_len = 16;
	int result;
	pthread_t writer_thread;
	pthread_t reader_thread;
	mt_insert_lookup_ctx_split writer_ctx;
	mt_insert_lookup_ctx_split reader_ctx;
	uint64_t buf_size = num_kvs * kv_required_size(max_key_len, DEFAULT_VALUE_SIZE);
	uint8_t* writer_buf = malloc(buf_size);
	uint8_t* reader_buf = malloc(buf_size);
	cuckoo_trie_split* trie = ct_alloc_split_view(num_kvs * 4);
	...
	result = pthread_create(&reader_thread, NULL, mt_insert_lookup_reader_thread_split_view, &reader_ctx);
	if (result != 0) {
		printf("Error creating reader thread\n");
		return;
	}

	result = pthread_create(&writer_thread, NULL, mt_insert_lookup_writer_thread_split_view, &writer_ctx);
	if (result != 0) {
		printf("Error creating reader thread\n");
		return;
	}
...
void* mt_insert_scan_writer_thread_split_view(void* arg) {
	uint64_t i;
	int result;
	uint8_t* buf_pos;
	mt_insert_scan_ctx_split* ctx = (mt_insert_scan_ctx_split*)arg;
...
void* mt_insert_scan_scan_thread_split_view(void* arg) {
	uint64_t iterations = 0;
	uint64_t kvs_seen;
	ct_kv* prev_kv = NULL;
	ct_kv* cur_kv = NULL;
	mt_insert_scan_ctx_split* ctx = (mt_insert_scan_ctx_split*)arg;
	ct_iter_split* iter = ct_iter_alloc_split_view(ctx->trie);

	// Use atomic_load to force re-reading writer_done every iteration.
	while (!__atomic_load_n(&(ctx->writer_done), __ATOMIC_ACQUIRE)) {
		ct_iter_goto_split_view(iter, 0, NULL);  // Place the iterator before all the keys

		kvs_seen = 0;
		while (1) {
			prev_kv = cur_kv;
			cur_kv = ct_iter_next_split_view(iter);
			if (cur_kv == NULL)
				break;  // Reached the maximal key in the trie
...
// Scan the whole trie while performing inserts
void test_mt_insert_scan_split_view(int verbose) {
	const uint64_t num_kvs = 10000;
	const uint64_t max_key_len = 16;
	int result;
	pthread_t scan_thread;
	pthread_t writer_thread;
	mt_insert_scan_ctx_split ctx;
	uint8_t* kvs_buf = malloc(num_kvs * kv_required_size(max_key_len, DEFAULT_VALUE_SIZE));
	cuckoo_trie_split* trie = ct_alloc_split_view(num_kvs * 4);
	...
	result = pthread_create(&scan_thread, NULL, mt_insert_scan_scan_thread_split_view, &ctx);
	if (result != 0) {
		printf("Error creating scan thread\n");
		return;
	}

	result = pthread_create(&writer_thread, NULL, mt_insert_scan_writer_thread_split_view, &ctx);
	if (result != 0) {
		printf("Error creating writer thread\n");
		return;
	}
...
void* mt_insert_succ_writer_thread_split_view(void* arg) {
	uint64_t i;
	int result;
	uint8_t* buf_pos;
	mt_insert_succ_ctx_split* ctx = (mt_insert_succ_ctx_split*)arg;
...
// Find the successor of random keys
void* mt_insert_succ_reader_thread_split_view(void* arg) {
	ct_kv* succ;
	uint64_t iters = 0;
	mt_insert_succ_ctx_split* ctx = (mt_insert_succ_ctx_split*)arg;
	uint64_t key_size;
	...
	while (!__atomic_load_n(&(ctx->writer_done), __ATOMIC_ACQUIRE)) {
		...
		ct_iter_goto_split_view(iter, key_size, key);
		succ = ct_iter_next_split_view(iter);
		if (succ && kv_key_compare_to(succ, key_size, key) < 0) {
			printf("Got incorrect successor!\n");
			printf("\tString:    ");
			print_bytes(key_size, key);
...
void test_mt_insert_succ_split_view(int verbose) {
	const uint64_t num_kvs = 10000;
	const uint64_t max_key_len = 16;
	int result;
	pthread_t writer_thread;
	pthread_t reader_thread;
	mt_insert_succ_ctx_split ctx;
	uint8_t* kvs_buf = malloc(num_kvs * kv_required_size(max_key_len, DEFAULT_VALUE_SIZE));
	cuckoo_trie_split* trie = ct_alloc_split_view(num_kvs);
	...
	result = pthread_create(&writer_thread, NULL, mt_insert_succ_writer_thread_split_view, &ctx);
	if (result != 0) {
		printf("Error creating writer thread\n");
		return;
	}

	result = pthread_create(&reader_thread, NULL, mt_insert_succ_reader_thread_split_view, &ctx);
	if (result != 0) {
		printf("Error creating reader thread\n");
		return;
	}
...
void* mt_insert_thread_split_view(void* arg) {
	uint64_t i;
	int ret;

	mt_insert_ctx_split* ctx = (mt_insert_ctx_split*) arg;
	uint8_t* buf_pos = ctx->kvs;
	...
	for (i = 0;i < ctx->num_kvs; i++) {
		...
		ret = ct_insert_split_view(ctx->trie, kv);
		if (ret == S_OVERFLOW) {
			// The mt-insert test expects the trie to be large enough to hold all keys, and
			// calls ct_verify_trie at the end. An overflow shouldn't happen.
			printf("Error: trie full after %lu keys.\n", i);
...
void test_mt_insert_split_view(char* dataset_name, int verbose) {
	const uint64_t num_kvs = 10000;
	const uint64_t num_threads = 8;

	int i;
	dataset_t dataset;
	mt_insert_ctx_split thread_contexts[num_threads];
	int result;
	pthread_t insert_threads[num_threads];
	cuckoo_trie_split* trie;
	...
	build_kvs(&dataset, DEFAULT_VALUE_SIZE);
	trie = ct_alloc_split_view(dataset.num_keys * 4);

	uint64_t workload_start = 0;
	uint64_t workload_end;
	for (i = 0; i < num_threads; i++) {
		mt_insert_ctx_split* ctx = &(thread_contexts[i]);
		workload_end = dataset.num_keys * (i+1) / num_threads;
		ctx->num_kvs = workload_end - workload_start;
		ctx->kvs = (uint8_t*) dataset.kv_pointers[workload_start];
		ctx->trie = trie;
	...
	for (i = 0;i < num_threads;i++) {
		result = pthread_create(&(insert_threads[i]), NULL, mt_insert_thread_split_view, &(thread_contexts[i]));
		if (result != 0) {
			printf("Error creating insert thread\n");
			return;
		}
...
int main(int argc, char** argv) {
	...
	if (!strcmp(test_name, "insert"))
		test_insert_split_view(verbose);
	else if (!strcmp(test_name, "iter"))
		test_iter_split_view();
	else if (!strcmp(test_name, "mt-insert-lookup"))
		test_mt_insert_lookup_split_view();
	else if (!strcmp(test_name, "mt-insert-scan"))
		test_mt_insert_scan_split_view(verbose);
	else if (!strcmp(test_name, "mt-insert-succ"))
		test_mt_insert_succ_split_view(verbose);
	else if (!strcmp(test_name, "mt-insert"))
		test_mt_insert_split_view(dataset_name, verbose);
	else
		printf("Unknown test name '%s'\n", test_name);
	return 0;
}
